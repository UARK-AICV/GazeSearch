    ...
    for task in task_names:
        ...
        for i, traj in enumerate(task_trajs):
            ...
            step_num = get_num_step2target(
                np.array(traj["X"]), np.array(traj["Y"]), bbox
            )
            step_num = len(traj["X"])
...
def get_center_keypoint_map(bbox, patch_num, box_size_dependent=True, normalize=True):
    ...
    if box_size_dependent:
        ...
        target_map = np.zeros((224, 224), dtype=np.float32)
...
def get_prior_maps(gt_scanpaths, im_w, im_h, visual_angle=24):
    ...
    for task in task_names:
        Xs = np.concatenate(
            [traj["X"][1:] for traj in gt_scanpaths if traj["task"] == task]
        )
        Ys = np.concatenate(
            [traj["Y"][1:] for traj in gt_scanpaths if traj["task"] == task]
        )
...
def transform_fixations(
    normalized_fixations, is_padding, PAD_IDX, EOS_IDX, n_special_symbols
):
