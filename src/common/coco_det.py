...
class COCODetHelper(object):
    ...
    def create_fixated_object_vector_target(
        self, img_name, fixation, output_h, output_w, fovea_size=16
    ):
        """Create the target vector for object recognition at a fixated location."""
        annos = self.get_annos_by_img_name(img_name, output_h, output_w)
        num_objs = len(annos["boxes"])
        target = np.zeros(self.num_classes, dtype=np.float32)
        fovea_box = {
            "x1": max(0, fixation[0] - fovea_size),
            "y1": max(0, fixation[1] - fovea_size),
            "x2": min(output_w, fixation[0] + fovea_size),
            "y2": min(output_w, fixation[1] + fovea_size),
        }
        for k in range(num_objs):
            cls_id = self.cat_ids[annos["labels"][k]]
            bbox = annos["boxes"][k]
            iom = get_IoM(fovea_box, dict(zip(["x1", "y1", "x2", "y2"], bbox)))
            target[cls_id] = iom
        if target.sum() > 0:
            target /= target.sum()
        return target

    def create_foveated_centermap_target(
        self, img_name, output_h, output_w, blur_slope
    ):
        annos = self.get_annos_by_img_name(img_name, output_h, output_w)
        hms = []
        for i in range(5):
            hm = (
                np.zeros((self.num_classes, output_h, output_w), dtype=np.float32)
                + i * blur_slope
            )
            num_objs = len(annos["boxes"])
            ...
            for k in range(num_objs):
                ...
                if h > 0 and w > 0:
                    ...
                    ct_int = ct.astype(np.int32)
                    hm[cls_id] = self.draw_umich_gaussian(
                        hm[cls_id], ct_int, radius, k=1 - i * blur_slope
                    )
            hms.append(hm)
