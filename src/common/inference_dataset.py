...
def process_data(
    ...
    valid_target_trajs_all = target_trajs_all
    ...
    if is_testing:
        ...
        test_target_trajs = list(filter(lambda x: x["split"] != "", target_trajs_all))
        ...
        train_target_trajs = target_trajs
        # print statistics
        ...
        valid_target_trajs = copy.deepcopy(target_trajs_all)
        # print statistics
        ...
        if hparams.Data.TAP in ["TP", "TAP"]:
            tp_trajs = list(
                filter(lambda x: x["condition"] == "present", target_trajs_all)
            )
            human_mean_cdf, _ = compute_search_cdf(
