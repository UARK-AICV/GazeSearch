...
def evaluate(
    ...
    print(
        f"Evaluating {TAP} with max steps to be {pa.max_traj_length} "
    ...
    # else:
    nonstop_scanpaths = actions2scanpaths(
        nonstop_actions, pa.patch_num, pa.im_h, pa.im_w
    )
    ...
    if TAP == "TP":
        ...
        metrics_dict.update(
            dict(zip([f"TFP_top{i}" for i in range(1, len(mean_cdf))], mean_cdf[1:]))
        )
    ...
    # Thang: we will need to define fix cluster for these SS scores
    # sequence score
    ...
    for traj in scanpaths:
        ...
        sp_len_diff.append(
            len(traj["X"]) - np.array([len(traj["X"]) for traj in gt_trajs])
        )
