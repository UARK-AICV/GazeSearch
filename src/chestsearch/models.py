...
class ExpertAttnTransformer(nn.Module):
    def __init__(
        ...
        # Task heads
        self.termination_predictor = MLP(
            hidden_dim + 1, hidden_dim, 1, num_output_layers
        )
        self.fixation_embed = MLP(hidden_dim, hidden_dim, hidden_dim, num_output_layers)
        if self.output_target_map:
            self.target_embed = MLP(
                hidden_dim, hidden_dim, hidden_dim, num_output_layers
            )
        if self.output_centermap:
            self.centermap_embed = MLP(
                hidden_dim, hidden_dim, hidden_dim, num_output_layers
            )
        if self.output_saliency:
            self.saliency_embed = MLP(
                hidden_dim, hidden_dim, hidden_dim, num_output_layers
            )
    ...
    def forward(
        ...
        if img_ids is not None and self.is_pretraining:
            ...
            bs = img.shape[0]
            high_res_featmaps = high_res_featmaps.unsqueeze(0).expand(
                bs, bs, *high_res_featmaps.shape[1:]
            )
            high_res_featmaps = high_res_featmaps.reshape(
                bs * bs, *high_res_featmaps.shape[2:]
            )
            l, bs, ch = dorsal_embs.shape
            dorsal_embs = (
                dorsal_embs.unsqueeze(1).expand(l, bs, bs, ch).reshape(l, bs * bs, ch)
            )
            dorsal_pos = (
                dorsal_pos.unsqueeze(1).expand(l, bs, bs, ch).reshape(l, bs * bs, ch)
            )
            scale_embs = (
                scale_embs.unsqueeze(1).expand(l, bs, bs, ch).reshape(l, bs * bs, ch)
            )
            tgt_len = tgt_seq_high.shape[1]
            tgt_seq = (
                tgt_seq.unsqueeze(1).expand(bs, bs, tgt_len).reshape(bs * bs, tgt_len)
            )
            tgt_seq_high = (
                tgt_seq_high.unsqueeze(1)
                .expand(bs, bs, tgt_len)
                .reshape(bs * bs, tgt_len)
            )
            tgt_padding_mask = (
                tgt_padding_mask.unsqueeze(1)
                .expand(bs, bs, tgt_len)
                .reshape(bs * bs, tgt_len)
            )
        ...
        if self.parallel_arch:
            ...
            padding_mask = torch.cat(
                [
                    torch.zeros(
                        bs, dorsal_embs.size(0), device=dorsal_embs.device
                    ).bool(),
                    tgt_padding_mask,
                ],
                dim=1,
        ...
        num_fixs = (
            (tgt_padding_mask.size(1) - torch.sum(tgt_padding_mask, dim=1))
            .unsqueeze(0)
            .expand(self.ntask, bs)
        )
        ...
        for i in range(self.num_decoder_layers):
            ...
            if not self.parallel_arch:
                ...
                query_embed, _ = self.transformer_cross_attention_layers_ventral[i](
                    query_embed,
                    ventral_embs,
                    memory_mask=None,
        ...
        if self.transfer_learning_setting == "none" or not self.project_queries:
            ...
            pred_queries = self.search2freeview_mlp(pred_queries).permute(2, 0, 1)
            pred_queries += self.new_query_embed.weight.unsqueeze(1)
            num_fixs = num_fixs[:1]
        elif self.transfer_learning_setting == "freeview2search":
            pred_queries = query_embed[: self.ntask].permute(1, 2, 0)
            pred_queries = self.freeview2search_mlp(pred_queries).permute(2, 0, 1)
            pred_queries += self.new_query_embed.weight.unsqueeze(1)
        ...
        outputs_fixation_map = torch.einsum(
            "lbc,bchw->lbhw", fixation_embed, output_featmaps
        )
        if self.training:
            ...
            outputs_fixation_map = F.interpolate(
                outputs_fixation_map.unsqueeze(1), size=(self.pa.im_h, self.pa.im_w)
            ).squeeze(1)
        ...
        if self.training:
            ind = self.ntask
            if self.output_centermap:
                centermap_embed = self.centermap_embed(query_embed[ind : ind + 80])
                out["pred_centermap"] = torch.einsum(
                    "lbc,bchw->lbhw", centermap_embed, high_res_featmaps
                ).transpose(0, 1)
                ind += 80
            if self.output_saliency:
                ...
                outputs_saliency = torch.einsum(
                    "lbc,bchw->lbhw", saliency_embed, high_res_featmaps
                )
            ...
            if self.output_target_map:
                ...
                out["pred_target_map"] = torch.einsum(
                    "lbc, bchw->lbhw", target_embed, high_res_featmaps
                ).transpose(0, 1)
    ...
    def decode_and_predict(
        ...
        if dorsal_mask is None:
            dorsal_mask = torch.zeros(
                1, *highres_embs.shape[1:], device=dorsal_embs.device
            )
        ventral_embs = torch.gather(
            torch.cat([dorsal_mask, highres_embs], dim=0),
            0,
            tgt_seq_high.unsqueeze(-1).expand(
                *tgt_seq_high.shape, highres_embs.size(-1)
            ),
        )
        ventral_pos = self.pixel_loc_emb(tgt_seq_high)  # Pos for fixation location
        ...
        out = {
            "pred_termination": output_termination.squeeze(-1)[
                task_ids, torch.arange(bs)
            ]
        }

        fixation_embed = self.fixation_embed(pred_queries)
        outputs_fixation_map = torch.einsum(
            "lbc,bchw->lbhw", fixation_embed, output_featmaps_wpos
        )
...
class FoveatedObjectMemory(nn.Module):
    def __init__(
        ...
        self.train_encoder = train_encoder
        self.encoder = ImageFeatureEncoder(
            pa.backbone_config, dropout, pa.pixel_decoder, load_segm_decoder=True
        )
        self.symbol_offset = len(self.pa.special_symbols)
        if not train_encoder:
            self.encoder.eval()
    ...
    def enhance_queries(self, queries, segm_results):
        ...
        masks = (
            ...
            .to(queries.device, dtype=torch.float32)
            .unsqueeze(1)
        )
    ...
    def encode(self, img: torch.Tensor):
        ...
        high_res_featmaps = self.input_proj(high_res_featmaps)
        working_memory, working_memory_pos, working_memory_mask = (
            self.construct_foveated_object_memory(segm_queries, segm_results)
        )
        return (
            working_memory,
            working_memory_pos,
    ...
    def construct_foveated_object_memory(self, queries, segm_results):
        ...
        fom_pos = None
        return (
            fom.view(-1, *fom.shape[2:]),
            fom_pos,
            fom_mask.reshape(-1, fom_mask.shape[-1]).transpose(0, 1),
        )
    ...
    def decode_and_predict(
        ...
        if working_memory_mask is None:
            working_memory_mask = torch.zeros(
                bs, working_memory.size(0), device=working_memory.device
            ).bool()
        padding_mask = (
            torch.cat([working_memory_mask, tgt_padding_mask], dim=1)
            if tgt_padding_mask is not None
            else None
        )
        ...
        if tgt_padding_mask is not None:
            ...
            num_fixs = torch.ones(1, bs).to(query_embed.device) * tgt_seq_high.size(0)

        attn_weights_all_layers = []
        for i in range(self.num_decoder_layers):
            query_embed, attn_weights = self.transformer_cross_attention_layers[i](
                query_embed,
                working_memory,
                memory_mask=None,
                memory_key_padding_mask=padding_mask,
        ...
        # Prediction
        x = torch.cat([query_embed[: self.ntask], num_fixs.unsqueeze(-1)], dim=-1)
        output_termination = self.termination_predictor(x)
        fixation_embed = self.fixation_embed(query_embed[: self.ntask])
        outputs_fixation_map = torch.einsum(
            "lbc,bchw->lbhw", fixation_embed, high_res_featmaps
        )
...
class HumanAttnTransformerV2(nn.Module):
    ...
    def construct_working_memory(
        ...
        ventral_embs = torch.gather(
            torch.cat(
                [
                    torch.zeros(1, *highres_embs.shape[1:], device=dorsal_embs.device),
                    highres_embs,
                ],
                dim=0,
            ),
        ...
        working_memory = self.working_memory_encoder(
            working_memory, src_key_padding_mask=padding_mask, pos=working_memory_pos
        )
    ...
    def decode_and_predict(
        ...
        fixation_ind_emb = (
            self.fix_ind_emb.weight[: tgt_seq_high.size(0)]
            .unsqueeze(1)
            .repeat(1, bs, 1)
        )
        ...
        query_mask = (
            torch.cat(
                [
                    torch.zeros(bs, 1, device=dorsal_embs.device).bool(),
                    tgt_padding_mask,
                ],
                dim=1,
            )
    ...
    def forward(
        ...
        dorsal_embs, dorsal_pos, dorsal_mask, high_res_featmaps = self.encode(img)
        out = self.decode_and_predict(
            dorsal_embs,
            dorsal_pos,
            dorsal_mask,
